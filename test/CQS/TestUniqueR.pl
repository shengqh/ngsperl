...
my $test = CQS::UniqueR->new();


#test result
my $expect_result = {
  'test' => ['r1/result/test.png','r1/result/test.csv'],
};
my $actual_result = $test->result( $config, "r1" );
is_deeply( $actual_result, $expect_result );

#test pbs
my $expect_pbs = {
  'test' => 'r1/pbs/test_uniqueR.pbs',
};
my $actual_pbs = $test->get_pbs_files( $config, "r1" );
is_deeply( $actual_pbs, $expect_pbs );

#test get_pbs_source
my $expect_pbs_source = { "r1/pbs/test_uniqueR.pbs" => [ "test", "test1", "test2" ], };
