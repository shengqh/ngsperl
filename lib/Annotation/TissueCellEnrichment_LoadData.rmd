...
cat("\n<mark>There are total", nrow(tau_df), "genes and", ncol(tau_df), "tissues in the tau file.</mark>\n\n")

cat(paste0("\n", top_ident, "# Overlap between tau genes and ", all_genes_label, " genes.\n\n"))
...
tau_overlap= intersect(rownames(tau_df), unique(all_genes))
cat(paste0("\n<mark>Among the ", length(all_genes), " genes in ", all_genes_label, ", ", length(tau_overlap), " were found to overlap with the Tau database, which comprises ", nrow(tau_df), " genes.</mark>\n\n"))
```

```{r, eval=perform_gtex}
cat(paste0("\n", top_ident, " Read protein atlas GTEx gene expression file\n\n"))
...
cat(paste0("\n", top_ident, "# Overlap between GTEx genes and ", all_genes_label, " genes\n\n"))
...
gtex_overlap= intersect(rownames(gtex_mat), unique(all_genes))
cat(paste0("\n<mark>Among the ", length(all_genes), " proteins identified in the physical functional proteome, ", length(gtex_overlap), " were found to overlap with the GTEx dataset, which comprises ", nrow(gtex_mat), " genes.</mark>\n\n"))
```

```{r eval=perform_tabula}
cat(paste0("\n", top_ident, " Load Tabula Sapiens snRNA dataset\n\n"))
...
cat("\n<mark>There are total", nrow(tabula_obj), "genes and", ncol(tabula_obj), "cells in the Tabula Sapiens snRNA dataset.</mark>\n\n")

cat(paste0("\n", top_ident, "# Overlap between Tabula Sapiens genes and ", all_genes_label, " genes\n\n"))
...
tabula_overlap= intersect(rownames(tabula_obj), unique(all_genes))
cat(paste0("\n<mark>Among the ", length(all_genes), " proteins identified in the physical functional proteome, ", length(tabula_overlap), " were found to overlap with the Tabula Sapiens dataset, which comprises ", nrow(tabula_obj), " genes.</mark>\n\n"))

cat(paste0("\n", top_ident, "# Cell type UMAP\n\n"))

if(exists("merge_fast_slow_muscle_cell")){
  if(merge_fast_slow_muscle_cell){
    cat("\n265 fast muscle cell and 111 slow muscle cell are combined as fast/slow muscle cell since they are overlapped at UMAP and hard to distinguish.\n")
    tabula_obj$cell_type[tabula_obj$cell_type=="fast muscle cell"] = "fast/slow muscle cell"
    tabula_obj$cell_type[tabula_obj$cell_type=="slow muscle cell"] = "fast/slow muscle cell"
  }
}
