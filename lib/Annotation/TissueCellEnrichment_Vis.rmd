...
if(!exists("top_n_celltype")){
  top_n_celltype=100
}

if(!exists("show_title")){
  show_title=TRUE
}
...
if(show_title){
  if(nchar(target_name) == 0){
    stop("show_title is TRUE but target_name is empty.")
  }
}

column_title=ifelse(show_title, target_name, character(0))
```

```{r eval=perform_tau}
cat(paste0("\n", top_ident, " Tau heatmap\n\n"))

genes_tau = tau_df[intersect(target_genes, rownames(tau_df)), ] |>
  dplyr::filter(tau > tau_threshold) |>
  dplyr::select(-tau)

cat("\nIn order to make the figure more readable, we only show ", nrow(genes_tau), " out of ", length(target_genes), " genes with tau > ", tau_threshold, ".")
...
cat(paste0("\n", top_ident, " Tissue specific gene count table\n\n"))

print_table(count_tbl, byDT=TRUE, row.names=FALSE)

cat(paste0("\n", top_ident, " Tissue specific gene Upset plot\n\n"))

valid_tissues = names(genes_list)[sapply(genes_list, length) > 0]
if(length(valid_tissues) == 0){
  cat("\nNo tissue with enriched genes found.\n")
}else{
  valid_genes_list = genes_list[valid_tissues]

  upset_png = paste0(file_prefix, ".tau.upset.png")
  png(upset_png, width=6, height=4.5, units="in", res=300) 
  g = upset(fromList(valid_genes_list), 
      sets = names(valid_genes_list),
      keep.order = FALSE,
      order.by = "freq",
...
```{r eval=perform_gtex}
cat(paste0("\n", top_ident, " Protein Atlas GTEx heatmap\n\n"))

genes_gtex = gtex_mat[intersect(target_genes, rownames(gtex_mat)), ]

if(nrow(genes_gtex) >= 30){
  cat("In order to make the figure more readable, we only show ", nrow(genes_gtex), " genes out of ", length(target_genes), " with max nTPM > 10.\n")
...
cat(paste0("\n", top_ident, " Tabula Sapiens Activity Score\n\n"))
...
cat(paste0("\n", top_ident, "# UMAP of Activity score\n\n"))

# in order to make the figures matching with the overall cell type UMAP,
# we will limit the cells to those in ct_obj with celltype > 1%, which is 
# generated in TissueCellEnrichment_LoadData.rmd
plot_obj=subset(score_obj, cells=colnames(ct_obj))

g=FeaturePlot_scCustom(plot_obj, 
...
ggsave(gsub(".png", ".pdf", ActivityScore_png), g, width=5, height=5, units="in", dpi=300, bg="white")
include_graphics(ActivityScore_png)

cat(paste0("\n", top_ident, "# Violin plot by tissue ranked by median activity score\n\n"))
...
ggsave(gsub(".png", ".pdf", ActivityScore_png), g, width=5, height=3, units="in", dpi=300, bg="white")
include_graphics(ActivityScore_png)       

cat(paste0("\n", top_ident, "# Violin plot by cell type ranked by median activity score\n\n"))

gd=FetchData(score_obj, c("cell_type", "ActivityScore1"))

vln_width=15
cts=names(sort(tapply(gd[,"ActivityScore1"], gd$cell_type, median), decreasing=TRUE))
if(length(cts) > top_n_celltype){
  cts=cts[1:top_n_celltype]
  cat("Showing top ", top_n_celltype, " cell types with highest median activity score.\n")
  gd=gd[gd$cell_type %in% cts,,drop=FALSE]
  vln_width=min(15, max(6, top_n_celltype * 0.3))
}

gd$cell_type=factor(gd$cell_type, levels=cts)
...
ggsave(ActivityScore_png, g, width=vln_width, height=5, units="in", dpi=300, limitsize=FALSE, bg="white")
ggsave(gsub(".png", ".pdf", ActivityScore_png), g, width=vln_width, height=5, units="in", dpi=300, limitsize=FALSE, bg="white")
include_graphics(ActivityScore_png)

cat(paste0("\n", top_ident, "# UMAP of Activity score and cell types\n\n"))
...
ggsave(gsub(".png", ".pdf", tabula_score_celltype_umap_png), g, width=20, height=6.5, units="in", dpi=300, bg="white")
