...
###1. Summary
This report includes quality check for raw sequencing data (section 2), reads mapping and assignment (section 3), and expression similarity between samples (section 4). The results of differential expression analysis are summarized in section 5 using the criteria of fold change > 2 & FDR<0.05.   

```{r include=FALSE, eval=hasEnrichmentAnalysis}
print("The top five enrichment pathways in KEGG are reported in section 6.")
```
...
```{r test, child = if (hasEnrichmentAnalysis) 'RNASeq_EnrichmentAnalysis.Rmd'} 
``` 
