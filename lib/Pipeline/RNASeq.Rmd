...
if(hasDEseq2 ){
  ...
  fpkm <- data.frame(fread(fpkm_file, drop=c(2:9), data.table=F), row.names=1)
  groups_def <- read.table("fileList6.txt", sep="\t")
  fpkm_t <- t(fpkm)
  ...
  for (i in 1:nrow(DESeq2_files)){
    ...
    if(nrow(deseq2) > 0){
      ...
      fpkm_agg2 <- fpkm_agg[,which(colnames(fpkm_agg) %in% comp),drop=FALSE]
