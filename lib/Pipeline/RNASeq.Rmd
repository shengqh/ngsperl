...
if(hasDEseq2 ){
  ...
  fpkm <- data.frame(fread(list.files(path="../../genetable/result/", pattern="fpkm.tsv$", full.names=T), drop=c(2:9), data.table=F), row.names=1)
  groups_def <- read.table("../../genetable/result/fileList2.txt", sep="\t")
  fpkm_t <- t(fpkm)
  fpkm2 <- merge(groups_def[,1:2], fpkm_t, by.x="V1", by.y="row.names", sort=F)
  ...
  for (i in 1:nrow(DESeq2_files)){
    ...
    if(nrow(deseq2) > 0){
      ...
      comp <- unlist(strsplit(gsub("deseq2_","", comparisonName), "_vs_|_VS_|_Vs_"))
      fpkm_agg2 <- fpkm_agg[,which(colnames(fpkm_agg) %in% comp==T),drop=FALSE]
      colnames(fpkm_agg2) <- paste0(colnames(fpkm_agg2), "_mean_FPKM")
