...
webgestaltFile<-"RNASeq_WebgestaltAnalysis.Rmd"
gseaFile<-"RNASeq_gseaAnalysis.Rmd"
...
if(hasWebGestaltAnalysis ){
  cat("####6.", enrichIndex, " WebGestalt\n", file=webgestaltFile)
  enrichIndex<-enrichIndex+1
}
if(hasGseaAnalysis ){
  cat("####6.", enrichIndex, " GSEA\n", file=gseaFile)
}
...
```{r enrich, child = if (hasWebGestaltAnalysis) webgestaltFile} 
```
...
```{r enrich, child = if (hasGseaAnalysis) gseaFile} 
```
...
if(file.exists(webgestaltFile)){
  file.remove(webgestaltFile)
}
if(file.exists(gseaFile)){
  file.remove(gseaFile)
}
