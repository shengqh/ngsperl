...
```{r child=webgestalt_rmd, eval=file.exists(webgestalt_rmd) }
```
...
`r if(hasFusion){paste0("Gene fusion was detected using STAR-Fusion package (", vmap['STAR-Fusion'], ")[8].")}`
...
`r if(hasFusion){"- [8] Haas, B.J., Dobin, A., Li, B. et al. Accuracy assessment of fusion transcript detection via read-mapping and de novo fusion transcript assembly-based methods. Genome Biol 2019, 20:213.\n"}`
