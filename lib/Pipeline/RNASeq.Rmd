...
# So, we cannot get how many unique mapped reads from the featureCounts summary.
  ...
  if(rnaseqOptions["featureCounts_UseMultiMappingReads", 1] == "FALSE"){
    validcol<-validcol[!(colnames(table)[validcol] %in% c("Unassigned_MultiMapping"))]
  }
...
if(rnaseqOptions["featureCounts_UseMultiMappingReads", 1] == "FALSE"){
  ...
  table<-table[!(colnames(table) %in% c("Unassigned_MultiMapping", "Total", "percent_assigned", "Assigned"))]
  table$Assigned <- assigned 
  table$Total <- rowSums(table[,c(2:ncol(table))]) 
  table$Percent_Assigned <- table$Assigned * 1.0 / table$Total 
  readsUsed = "**UNIQUE** reads"
}
colsum<-apply(table[,c(2:ncol(table))], 2, sum)
validcol<-which(colsum > 0)+1
output_table2<-data.frame(table[,c(1, validcol)])
print(kable(output_table2, caption=tabRef("geneSummary", paste0("The summary of ", readsUsed, " assignment to genes"))))
```
