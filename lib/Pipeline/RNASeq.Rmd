...
if (any(table$Mean<=QualityMin)) { #use this as cutoff to show the quality table
  ...
  print(kable_styling(kable(output_table1, caption=tabRef("Quality Summary", "Samples with Low RNA-seq Quality"), row.names=F) %>% 
...
```{r fastqc_raw_per_sequence_gc_content_table,echo=FALSE,results='asis', include = hasFastqc, eval = hasFastqc,warning=FALSE}
...
if (any(gcSampleInd)) { #use this as cutoff to show GC distribution
  ...
  print(kable_styling(kable(output_table1, caption=tabRef("GC content Distribution Summary", "Samples with abnormal GC content Distribution"), row.names=F) %>% 
          row_spec(which(apply(output_table1[,-1],1,min)< QualityMin), color = "black", background = "honeydew")))
}
...
print(kable_styling(kable(df, caption=tabRef("versionFiles", "Software versions"), row.names=F)))
