...
if("all_correlation_heatmap" %in% names(file_map)){
  heatmap_png=file_map$all_correlation_heatmap
  biotype_png = gsub(".heatmap.png", ".biotype_perc.png", gsub(".proteincoding.count", ".count", heatmap_png))
  has_biotype_png = file.exists(biotype_png)
}else{
  biotype_png=""
  has_biotype_png = FALSE
}
...
cat("\n### Genes detected in sample\n")
include_graphics(file_map$featureCounts_gene_png)
```

```{r, echo=FALSE,results='asis', eval=has_biotype_png, fig.align="center", fig.align="center", fig.cap=figRef("has_biotype_png", "Gene biotype distribution of each sample")}
cat("\n### Gene biotype distribution\n")
include_graphics(biotype_png)
```
...
figureRmd<-function(file_map, corNames, section, hca_gene_used){
  ...
  for(i in c(1:length(corNames))){
    ...
    if(file.exists(corr_heatmap)){
      result<-paste0(result, get_figure_description(basename(corr_heatmap), corr_heatmap, paste(name, "sample correlation")))
    }
  }
  return(result)
}
