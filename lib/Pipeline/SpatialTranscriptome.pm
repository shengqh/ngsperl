...
",
  ...
  if ( $def->{perform_SpaGene} ) {
    ...
    $config->{$spagene_task} = {
      ...
      output_ext => ".SpaGene.object.rds,.SpaGene.pattern.rds,.SpaGene.LR.rds",
      pbs        => {
        "nodes"    => "1:ppn=1",
        "walltime" => "24",
        "mem"      => "300gb"
  ...
  if ( $def->{perform_RCTD} ) {
    ...
    for my $assay ( @{$assays} ) {
      ...
      $config->{$rctd_task} = {
        ...
        perform                  => 1,
        option                   => "",
        rtemplate                => "reportFunctions.R,../scRNA/scRNA_func.r,../scRNA/Deconvolution_functions.R,../scRNA/Deconvolution_RCTD_obj.r",
        parameterSampleFile1_ref => $source_def,
        parameterSampleFile2     => {
          "assay"       => $assay,
  ...
  if ( getValue( $def, "perform_MEcell", 0 ) ) {
    ...
    for ( my $i = 0; $i < scalar( @{$MEcell_resolutions} ); $i++ ) {
      ...
      $config->{$MEcell_cluster_task} = {
        ...
        no_docker                => getValue( $def, "no_docker", 0 ),
        output_to_same_folder    => 0,
        output_ext               => ".$source_assay.MEcell.res.$cur_resolution.$cluster_algorithm_name.meta.rds",
    ...
    if ( $def->{perform_subcluster} ) {
      ...
      $config->{$subcluster_task} = {
        ...
        rReportTemplate          => "../scRNA/spatial_MEcell_sub_cluster.Rmd;reportFunctions.R",
        run_rmd_independent      => 1,
        rmd_ext                  => ".spatial_MEcell_sub_cluster.html",
        option                   => "",
