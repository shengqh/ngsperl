...
our %EXPORT_TAGS = ( 'all' => [qw(performPIPseq3)] );
...
sub getConfig {
  ...
  my $pipseeker_command = getValue($def, "pipseeker_command");
  my $pipseeker_chemistry = getValue($def, "pipseeker_chemistry");
...
IFS=, read -r var1 var2 <<< __FILE__
DIR=\"\$(dirname \"\${var1}\")\"
FILE=\"\$(basename \"\${var1}\")\"
FILE_PREFIX=\"\$(echo \$FILE | sed -e \"s/_S.*\$//\")\"

echo $pipseeker_command full --chemistry $pipseeker_chemistry --fastq \${DIR}/\${FILE_PREFIX} --id __NAME__ --output-path ${target_dir}/${pipseeker}/result/__NAME__ --star-index-path $pipseeker_star_index --threads 8
$pipseeker_command full --chemistry $pipseeker_chemistry --fastq \${DIR}/\${FILE_PREFIX} --id __NAME__ --output-path ${target_dir}/${pipseeker}/result/__NAME__ --star-index-path $pipseeker_star_index --threads 8
...
",
  ...
  if(getValue($def, "perform_individual_qc", 0)){
    ...
    my ($raw_individual_qc_task, $qc_report_task, $annotation_dic) = add_individual_qc_tasks($config, $def, $summary, $target_dir, $task_name, "", undef, [$count_task_name, $qc_pattern], undef, undef);
  }
