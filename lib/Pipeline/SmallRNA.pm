...
sub getSmallRNAConfig {
	...
	my ( $config, $individual_ref, $summary_ref, $cluster, $not_identical_ref,
		$preprocessing_dir, $class_independent_dir )
	  = getPrepareConfig( $def, 1 );

	my $host_genome_dir =
	  create_directory_or_die( $def->{target_dir} . "/host_genome" );
	my $nonhost_library_dir =
	  create_directory_or_die( $def->{target_dir} . "/nonhost_library" );
	my $nonhost_genome_dir =
	  create_directory_or_die( $def->{target_dir} . "/nonhost_genome" );
	my $nonhost_blast_dir =
	  create_directory_or_die( $def->{target_dir} . "/nonhost_blast" );
	my $data_visualization_dir =
	  create_directory_or_die( $def->{target_dir} . "/data_visualization" );
	...
	my $blast_unmapped_reads =
	  defined $def->{blast_unmapped_reads} && $def->{blast_unmapped_reads};
	my $do_comparison           = defined $def->{pairs};
	my $groups                  = $def->{groups};
	my $groups_vis_layout       = $def->{groups_vis_layout};
	my $add_count_one_in_DEseq2 = ( !defined $def->{add_count_one_in_DEseq2} )
	  || $def->{add_count_one_in_DEseq2};
	...
	if ($do_comparison) {
		my $class_independent = {
			deseq2_top100_reads => {
				...
				groups_ref => "groups",
				countfile_ref =>
				  [ "identical_sequence_count_table", ".read.count\$" ],
				sh_direct            => 1,
			...
			deseq2_top100_contigs_vis => {
				...
				parameterSampleFile1_ref =>
				  [ "deseq2_top100_contigs", "_DESeq2.csv\$" ],
				parameterSampleFile2 => $def->{pairs_top100_deseq2_vis_layout},
				sh_direct            => 1,
				pbs                  => {
					"email"    => $def->{email},
		...
		push @summary,
		  (
			"deseq2_top100_reads",     "deseq2_top100_contigs",
			"deseq2_top100_reads_vis", "deseq2_top100_contigs_vis"
		  );
	...
	if ($search_host_genome) {
		...
		my $host_genome = {
			...
			bowtie1_genome_1mm_NTA_smallRNA_table => {
				...
				option => "",
				source_ref =>
				  [ "bowtie1_genome_1mm_NTA_smallRNA_count", ".mapped.xml" ],
				cqs_tools => $def->{cqstools},
			...
			host_genome_tRNA_category => {
				...
				rCode => 'maxCategory=3;textSize=9;groupTextSize='
				  . $def->{table_vis_group_text_size} . ';',
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
		...
		push @table_for_pieSummary,
		  ( "bowtie1_genome_1mm_NTA_smallRNA_count", ".count\$" );
		push @table_for_correlation,
		  ( "bowtie1_genome_1mm_NTA_smallRNA_table", ".count\$" );
		...
		push @individual,
		  ( "bowtie1_genome_1mm_NTA", "bowtie1_genome_1mm_NTA_smallRNA_count" );
		push @summary,
		  (
			"bowtie1_genome_1mm_NTA_smallRNA_table",
			"bowtie1_genome_1mm_NTA_smallRNA_category",
		...
		if ($do_comparison) {
			my $comparison = {
				...
				host_genome_deseq2_vis => {
					...
					parameterSampleFile1_ref => [
						"deseq2_miRNA",         "_DESeq2.csv\$",
						"deseq2_tRNA",          "_DESeq2.csv\$",
						"deseq2_otherSmallRNA", "_DESeq2.csv\$"
					],
			...
			push @summary,
			  (
				"deseq2_miRNA",          "deseq2_tRNA",
				"deseq2_tRNA_aminoacid", "deseq2_otherSmallRNA",
				"host_genome_deseq2_vis"
			  );
		}
		...
		my $unmapped_reads = {
			...
			bowtie1_genome_unmapped_reads => {
				...
				output_other_ext =>
				  "_clipped_identical.unmapped.fastq.dupcount",
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
					"nodes"    => "1:ppn=1",
		...
		push @individual,
		  ( "bowtie1_genome_1mm_NTA_pmnames", "bowtie1_genome_unmapped_reads" );
		push @table_for_pieSummary,
		  ( "bowtie1_genome_unmapped_reads", ".dupcount" );
	...
	if ($search_miRBase) {
		my $mirbase = {
			...
			bowtie1_miRBase_pm_count => {
				...
				perform    => 1,
				target_dir => $nonhost_library_dir
				  . "/bowtie1_miRBase_pm_count",
				option => $def->{mirbase_count_option}
				  . " -m --keepChrInName --keepSequence",
		...
		my $unmappedreads = {
			...
			nonhost_library_tRNA_vis => {
				...
				target_dir => $data_visualization_dir
				  . "/nonhost_library_tRNA_vis",
				rtemplate =>
				  "countTableVisFunctions.R,bacteriaTrnaMappingVis.R",
				output_file               => ".tRNAMapping.Result",
			...
			bowtie1_rRNAL_pm_count => {
				...
				option     => $def->{smallrnacount_option}
				  . ' --keepChrInName --keepSequence --categoryMapFile '
				  . $def->{rrnaL_category_map},
			...
			nonhost_library_rRNAL_vis => {
				...
				parameterFile3_ref => [ "fastqc_count_vis", ".Reads.csv\$" ],
				rCode => 'maxCategory=NA;textSize=9;groupTextSize='
				  . $def->{table_vis_group_text_size} . ';',
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
			...
			nonhost_genome_bacteria_group1_vis => {
				...
				parameterSampleFile2      => $groups_vis_layout,
				parameterFile1_ref =>
				  [ "bowtie1_bacteria_group1_pm_table", ".count\$" ],
				parameterFile2     => $def->{bacteria_group1_log},
				parameterFile3_ref => [ "fastqc_count_vis", ".Reads.csv\$" ],
				rCode              => 'maxCategory=4;textSize=9;groupTextSize='
				  . $def->{table_vis_group_text_size} . ';',
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
			...
			nonhost_genome_bacteria_group2_vis => {
				...
				parameterSampleFile2      => $groups_vis_layout,
				parameterFile1_ref =>
				  [ "bowtie1_bacteria_group2_pm_table", ".count\$" ],
				parameterFile2     => $def->{bacteria_group2_log},
				parameterFile3_ref => [ "fastqc_count_vis", ".Reads.csv\$" ],
				rCode              => 'maxCategory=5;textSize=9;groupTextSize='
				  . $def->{table_vis_group_text_size} . ';',
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
			...
			nonhost_genome_fungus_group4_vis => {
				...
				parameterSampleFile2      => $groups_vis_layout,
				parameterFile1_ref =>
				  [ "bowtie1_fungus_group4_pm_table", ".count\$" ],
				parameterFile2     => $def->{fungus_group4_log},
				parameterFile3_ref => [ "fastqc_count_vis", ".Reads.csv\$" ],
				sh_direct          => 1,
				rCode              => 'maxCategory=8;textSize=9;groupTextSize='
				  . $def->{table_vis_group_text_size} . ';',
				pbs => {
					"email"    => $def->{email},
					"nodes"    => "1:ppn=1",
			...
			nonhost_overlap_vis => {
				...
				parameterSampleFile1_ref => [
					"bowtie1_bacteria_group1_pm_table", ".read.count\$",
					"bowtie1_bacteria_group2_pm_table", ".read.count\$",
					"bowtie1_fungus_group4_pm_table",   ".read.count\$",
					"bowtie1_tRNA_pm_table",              ".read.count\$",
					"bowtie1_rRNAL_pm_table",             ".read.count\$",
					"bowtie1_rRNAS_pm_table",             ".read.count\$",
				],
				parameterSampleFile2Order => $def->{groups_order},
				parameterSampleFile2      => $groups,
				parameterSampleFile3      => $groups_vis_layout,
...
#				parameterFile2            => $def->{fungus_group4_log},
		...
		push @table_for_correlation,
		  (
			"bowtie1_tRNA_pm_table",              ".count\$",
			"bowtie1_rRNAL_pm_table",             ".count\$",
			"bowtie1_rRNAS_pm_table",             ".count\$",
			"nonhost_genome_bacteria_group1_vis", ".Species.csv\$",
			"nonhost_genome_bacteria_group2_vis", ".Species.csv\$",
			"nonhost_genome_fungus_group4_vis",   ".Species.csv\$"
		  );
		push @table_for_countSum,
		  (
			"bowtie1_tRNA_pm_table",            ".category.count\$",
			"bowtie1_rRNAL_pm_table",           ".count\$",
			"bowtie1_rRNAS_pm_table",           ".count\$",
			"bowtie1_bacteria_group1_pm_table", ".count\$",
			"bowtie1_bacteria_group2_pm_table", ".count\$",
		...
		push @individual,
		  (
			"bowtie1_tRNA_pm",            "bowtie1_tRNA_pm_count",
			"bowtie1_rRNAL_pm",           "bowtie1_rRNAL_pm_count",
			"bowtie1_rRNAS_pm",           "bowtie1_rRNAS_pm_count",
			"bowtie1_bacteria_group1_pm", "bowtie1_bacteria_group1_pm_count",
			"bowtie1_bacteria_group2_pm", "bowtie1_bacteria_group2_pm_count",
		...
		push @mapped,
		  (
			"bowtie1_tRNA_pm_count",            ".xml",
			"bowtie1_rRNAL_pm_count",           ".xml",
			"bowtie1_rRNAS_pm_count",           ".xml",
			"bowtie1_bacteria_group1_pm_count", ".xml",
			"bowtie1_bacteria_group2_pm_count", ".xml",
			"bowtie1_fungus_group4_pm_count",   ".xml"
		  );
		...
		if ($do_comparison) {
			my $unmapped_comparison = {
				...
				nonhost_library_deseq2_vis => {
					...
					output_file_ext          => ".DESeq2.Matrix.png",
					parameterSampleFile1_ref => [
						"deseq2_nonhost_tRNA",  "_DESeq2.csv\$",
						"deseq2_nonhost_rRNAL", "_DESeq2.csv\$",
						"deseq2_nonhost_rRNAS", "_DESeq2.csv\$"
					],
					parameterSampleFile2 =>
					  $def->{pairs_nonHosttRNArRNA_deseq2_vis_layout},
					sh_direct => 1,
					pbs       => {
						"email"    => $def->{email},
						"nodes"    => "1:ppn=1",
				...
				nonhost_genome_deseq2_vis => {
					...
					output_file_ext          => ".DESeq2.Matrix.png",
					parameterSampleFile1_ref => [
						"deseq2_bacteria_group1", "_DESeq2.csv\$",
						"deseq2_bacteria_group2", "_DESeq2.csv\$",
						"deseq2_fungus_group4",   "_DESeq2.csv\$"
					],
					parameterSampleFile2 =>
				...
				nonhost_genome_deseq2_reads_vis => {
					...
					output_file_ext          => ".DESeq2.Matrix.png",
					parameterSampleFile1_ref => [
						"deseq2_bacteria_group1_reads", "_DESeq2.csv\$",
						"deseq2_bacteria_group2_reads", "_DESeq2.csv\$",
						"deseq2_fungus_group4_reads",   "_DESeq2.csv\$"
					],
					parameterSampleFile2 =>
					  $def->{pairs_nonHostGroups_deseq2_vis_layout},
					sh_direct => 1,
	...
	if ($blast_unmapped_reads) {
		my $blast = {
			...
			bowtie1_unmapped_sequence_blast => {
				...
				option => "",
				source_ref =>
				  [ "bowtie1_unmapped_sequence_count_table", ".fasta\$" ],
				sh_direct => 0,
				cluster   => $cluster,
				pbs       => {
					"email"    => $def->{email},
	...
	push @summary,
	  ( "count_table_correlation", "reads_in_tasks", "reads_in_tasks_pie" );
