...
our %EXPORT_TAGS =
  ( 'all' => [qw(getSmallRNADefinition getPrepareConfig saveConfig)] );
...
sub getPrepareConfig {
	...
	my $fastq_remove_N = !defined $def->{fastq_remove_N}
	  || $def->{fastq_remove_N};
	my $run_cutadapt = !defined $def->{run_cutadapt} || $def->{run_cutadapt};
	my $fastq_remove_random = !defined $def->{fastq_remove_random}
	  || $def->{fastq_remove_random};
	...
	if ($run_cutadapt) {
		...
		if ( !defined $cutadapt_option ) {
			$cutadapt_option = "-O 10 -m " . $def->{min_read_length};
		}
		...
		push @summary,
		  ( "fastqc_pre_trim_summary", "fastqc_post_trim_summary" );
		...
		if ($fastq_remove_random) {
			$config->{"removeRandom"} = {
				...
				option     => "-u ".$fastq_remove_random." -u -".$fastq_remove_random,,
				source_ref => $source_ref,
#				adapter    => $adapter,
		...
		if ( !defined $def->{remove_sequences} ) {
			$config->{"fastqc_count_vis"} = {
				...
				parameterFile1_ref =>
				  [ "fastqc_pre_trim_summary", ".FastQC.reads.tsv\$" ],
				parameterFile2_ref =>
				  [ "fastqc_post_trim_summary", ".FastQC.reads.tsv\$" ],
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
					"nodes"    => "1:ppn=1",
		...
		if ( !defined $run_cutadapt || $run_cutadapt ) {
			$config->{"fastqc_count_vis"} = {
				...
				parameterFile2_ref =>
				  [ "fastqc_post_trim_summary", ".FastQC.reads.tsv\$" ],
				parameterFile3_ref =>
				  [ "fastqc_post_remove_summary", ".FastQC.reads.tsv\$" ],
				sh_direct => 1,
				pbs       => {
					"email"    => $def->{email},
					"nodes"    => "1:ppn=1",
