...
```{r}
levels=unique(levels_df$Level)
for(level in levels){
  level_df = levels_df |> dplyr::filter(Level == level)
  cat("\n#### Level: ", level, "\n\n")
  print(htmltools::tagList(print_table(level_df, byDT=TRUE, row.names=F, pageLength = 20)))
}
```

### Coverage proportion of sites at different levels
...
cov_width = get_bar_width_with_legend(length(unique(cov_perc_depth$sample)))
cov_height = 4
...
### The percentage of methylated CpG sites (fractional_meth)

The percentage of methylated sites represents the percentage of CpG sites that are determined to be methylated among all sites with sufficient coverage for methylation calling.
...
```{r fig.cap=figRef("methy_summary1", "The percentage of methylated sites")}
include_graphics(fra_png)
```

### the mean of the methylation levels for covered sites (mean_meth)
...
mean_width=get_bar_width_no_legend(nsample)
mean_height=4
...
### the mean of the methylation levels weighted by coverage (mean_meth_weighted)
...
The CpG β value represents the fraction of methylated reads among all reads covering a CpG site, calculated as the ratio of methylated cytosines to the total number of cytosines (methylated + unmethylated) observed at that site.

## Distribution

CpG β value closing to 0 indicates low methylation, while values near 1 indicate high methylation.

We should observe two peaks in the histogram, one near 0 (unmethylated) and another near 1 (methylated), 
reflecting the bimodal distribution of DNA methylation in mammalian genomes.

If you do not see this expected pattern, it may indicate potential issues with the bisulfite conversion efficiency, sequencing quality, or data processing steps.
