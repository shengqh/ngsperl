...
our %EXPORT_TAGS = (
  'all' => [
    qw(initializeScRNASeqDefaultOptions
        performScRNASeq
        performScRNASeqTask)
  ]
);
...
sub initializeScRNASeqDefaultOptions {
  ...
  initDefaultValue( $def, "remove_subtype", "T cells,B cells,Plasma cells,Fibroblasts,Neurons,Epithelial cells,Endothelial cells,Macrophages,Dendritic cells,Ciliated cells" ), initDefaultValue( $def, "best_resolution_min_markers", 20 );
...
sub getScRNASeqConfig {
  ...
  if ( defined $def->{files} ) {
    ...
    if ($remove_doublets) {
      ...
      if ( defined $scDblFinder_task ) {
        $files_def = [ $scDblFinder_task, ".scDblFinder.singlet_object.rds" ];
        $prefix    = "${scDblFinder_task}_";
      }
      else {
        ...
        if ( defined $doublets_ref ) {
          ...
          $prefix    = "${prefix}nd_";
        } ## end if ( defined $doublets_ref)
      } ## end else [ if ( defined $scDblFinder_task)]
    ...
    if ($perform_split_hto_samples) {
    ...
    } ## end if ($perform_split_hto_samples)
    else {
      if ( $perform_arcasHLA || $perform_strelka2 ) {
      ...
      } ## end if ($perform_arcasHLA)
    } ## end else [ if ($perform_split_hto_samples)]
    ...
    if ( getValue( $def, "perform_seurat" ) ) {
      ...
      if ( getValue( $def, "rawdata_from_object", 0 ) ) {
        ...
        if ( getValue( $def, "rawdata_from_qc", 0 ) ) {
          ...
          $is_preprocessed = 1;
        } ## end if ( getValue( $def, "rawdata_from_qc"...))
        else {
          ...
          $is_preprocessed = 0;
        } ## end else [ if ( getValue( $def, "rawdata_from_qc"...))]
        ...
        push( @report_files, ( $seurat_task, ".final.png", $seurat_task, ".qc.1.png", $seurat_task, ".qc.2.png", $seurat_task, ".qc.3.png", $seurat_task, ".qc.4.png", $seurat_task, ".sample_cell.csv" ) );
        push( @report_names, ( "seurat_merge_png", "seurat_qc_1_png", "seurat_qc_2_png", "seurat_qc_3_png", "seurat_qc_4_png", "sample_cell_csv" ) );

        $localization_ref = [ $seurat_task, ".final.rds" ];
      } ## end else [ if ( getValue( $def, "rawdata_from_object"...))]
      ...
      if ( getValue( $def, "perform_dynamic_cluster" ) ) {
        ...
        if ( getValue( $def, "perform_dynamic_subcluster" ) ) {
          ...
          if ( defined $def->{dynamic_delete_celltypes} ) {
            ...
            } ## end for my $dct (@$delete_cts)
          } ## end if ( defined $def->{dynamic_delete_celltypes...})

          $subcluster_task = addSubCluster( $config, $def, $tasks, $target_dir, $subcluster_task, $obj_ref, $meta_ref, $essential_gene_task, $cur_options, $rename_map, ".dynamic_subcluster.html", $signacX_ref, $singleR_ref, $azimuth_ref, $celltypist_ref );
          ...
          if ( getValue( $def, "perform_dynamic_choose" ) ) {
            ...
            # }
          } ## end if ( getValue( $def, "perform_dynamic_choose"...))
        } ## end if ( getValue( $def, "perform_dynamic_subcluster"...))
      } ## end if ( getValue( $def, "perform_dynamic_cluster"...))
      ...
      } ## end if ( getValue( $def, "perform_localization_genes_plot"...))
      ...
      if ( getValue( $def, "perform_localization_gene_ratio_plot", 0 ) ) {
        ...
        my $genes                            = { $def->{task_name} => getValue( $def, "localization_gene_ratio" ), };
        #print(Dumper($genes));
      ...
      } ## end if ( getValue( $def, "perform_localization_gene_ratio_plot"...))
      ...
      if ( getValue( $def, "perform_multires", 0 ) ) {
        ...
        if ( getValue( $def, "perform_multires_subcluster", 0 ) ) {
          ...
          } ## end if ( getValue( $def, "perform_multires_choose"...))
        } ## end if ( getValue( $def, "perform_multires_subcluster"...))
      } ## end if ( getValue( $def, "perform_multires"...))
      ...
      if ( getValue( $def, "perform_fix_resolution" ) ) {
        ...
        } ## end if ( getValue( $def, "perform_heatmap"...))
        ...
        push( @report_files, ( $cluster_task, ".final.rds", $celltype_task, ".celltype.csv", $celltype_task, ".celltype.rds", $celltype_task, ".summary_layer.png", $celltype_task, ".cluster.png", $celltype_task, ".celltype.group.label.png", $celltype_task, ".score.png" ) );
        ...
        if ( getValue( $def, "plot_gsea_genes", 0 ) ) {
          ...
          $config->{$gesa_genes_task} = {
            ...
            parameterSampleFile1 => { by_sctransform => getValue( $def, "by_sctransform" ), },
            parameterFile1_ref   => [ $cluster_task,  ".final.rds" ],
        ...
        } ## end if ( getValue( $def, "plot_gsea_genes"...))
        ...
        } ## end if ( getValue( $def, "perform_CHETAH"...))
        ...
        if ( getValue( $def, "perform_rename_cluster" ) ) {
          ...
          $config->{$rename_cluster} = {
            ...
            parameterFile3_ref   => [ $celltype_task, ".celltype.csv" ],
            parameterSampleFile1 => { celltype_name => $celltype_name },
          ...
          addGeneTask( $config, $def, $tasks, $target_dir, $cluster_task, $celltype_task, $celltype_cluster_file, $celltype_name, $cluster_name );
        } ## end if ( getValue( $def, "perform_rename_cluster"...))
        ...
        } ## end if ( defined $def->{bubblemap_file...})
        ...
        } ## end if ( getValue( $def, "perform_recluster"...))
        ...
        } ## end if ( $def->{perform_marker_dotplot...})
        ...
        if ( $def->{perform_curated_gene_dotplot} ) {
          ...
          if ( defined $curated_gene_files ) {
            ...
            } ## end for my $cf ( keys %$curated_gene_files)
          } ## end if ( defined $curated_gene_files)
        ...
        } ## end if ( $def->{perform_curated_gene_dotplot...})
      ...
      } ## end if ( getValue( $def, "perform_fix_resolution"...))
