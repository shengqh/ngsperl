...
has_homer<-"homer_result" %in% rownames(chipseqOptions)
if(has_homer) {
  homer <- chipseqOptions["homer_result", 1]
}

has_diffbind<-"diffbind_result" %in% rownames(chipseqOptions)
if(has_diffbind) {
  diffbind <- chipseqOptions["diffbind_result", 1]
}
...
#motif_files <- c(paste0("[", basename(homer_png), "](",homer_png, ")"))
...
```{r Correlation,echo=FALSE,results='asis', eval=has_chipqc, fig.align="center", fig.cap=figRef("peak_correlation", "Plot of correlation between peaksets"), out.width="40%", out.height="40%"}
include_graphics(list.files(chipqc, pattern="PeakCorHeatmap.png", full.names=T, recursive=T))
```

```{r PCA,echo=FALSE,results='asis', eval=has_chipqc, fig.align="center", fig.cap=figRef("pca", "PCA of peaksets"), out.width="40%", out.height="40%"}
