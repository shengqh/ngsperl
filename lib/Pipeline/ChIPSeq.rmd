...
```{r genomic_features,echo=FALSE,warning=FALSE, results='asis', eval=has_chipqc, fig.align="center", fig.cap=figRef("feature_enrichment", "Heatmap of log2 enrichment of reads in genomic features"), out.width="40%", out.height="20%"}
cat("## Enrichment of reads in genomic features\n")
check_and_include_graphics(files$chipqc_GenomicFeatureEnrichment)
```

<br>

```{r CrossCoverage,echo=FALSE,warning=FALSE, results='asis', eval=has_chipqc, fig.align="center", fig.cap=figRef("cross_coverage", "Plot of CrossCoverage score after successive strand shifts"), out.width="40%", out.height="20%"}
cat("## ChIP signal distribution and structure\n")
check_and_include_graphics(files$chipqc_CCPlot)
```
...
check_and_include_graphics(files$chipqc_PeakCorHeatmap)
cat("* Replicate samples of high quality can be expected to cluster together in the heatmap and be spatially grouped within the PCA plot.\n\n")
```
...
check_and_include_graphics(files$chipqc_PeakPCA)
```
