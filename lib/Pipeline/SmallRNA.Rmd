...
myoptions<-read.table("fileList2.txt", header=FALSE, as.is=TRUE)
...
categoryRmd<-function(dfCategory, fileName, header){
  ...
  for(i in c(1:nrow(dfCategory))){
    ...
    if(file.exists('fileList7.txt')){
      comparisons = unique(read.table('fileList7.txt', sep="\t")$V2)
...
```{r, echo=FALSE, results='asis'}
if(hasRefBacteriaAgg){
  cat("# Refseq bacteria")
}
```

```{r refBacteriaAgg, include=hasRefBacteriaAgg, child=nonhostBacteriaAggFile} 
```

```{r refBacteriaEst, include=hasRefBacteriaEst, child=nonhostBacteriaEstFile} 
```
...
if(normalize_by_read == "FeatureReads"){
  ...
  normalize_by_read = "total mappable reads"
}
```
...
Briefly, Cutadapt (`r vmap['Cutadapt']`)[2] was used to trim 3’ adapters for raw reads. 
All reads with less than 16 nucleotides were designated as “too short” and discarded. 
Quality control on both raw reads and adaptor-trimmed reads was performed using FastQC (`r vmap['FastQC']`)(www.bioinformatics.babraham.ac.uk/projects/fastqc). 
The adaptor-trimmed reads were mapped to the `r vmap['host']` genome, addition to rRNA and tRNA reference sequences, 
by Bowtie1 (`r vmap['bowtie']`)[3] allowing only one mismatch. 
`r if(hasHostDEseq2){paste0("Significantly differential expressed features with ", deseq2criteria, " were detected by DESeq2 (", vmap['DESeq2'], ")[4] using ", normalize_by_read, " as normalization factor.")}` 
