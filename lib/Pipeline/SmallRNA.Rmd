...
hasLengthDistCategory<-file.exists(files["host_length_dist_category",1])

hasAdapterRaw<-file.exists(files["fastqc_raw_adapter_content",1])
hasAdapterPostTrim<-file.exists(files["fastqc_post_trim_adapter_content",1])
...
```{r,echo=FALSE,results='asis', fig.align="center", fig.cap=figRef("fastq_len", "Read length distribution after adapter trimming", trunk.eval=(file.exists(files["fastq_len",1]) & (!hasLengthDistCategory)))}
...
```{r bacteria_count_vis,echo=FALSE,results='asis', fig.align="center", fig.cap=figRef("bacteria_count_vis", "Bacteria count", trunk.eval=file.exists(files["bacteria_count_vis",1]))}
cat('## Non-Host bacteria count\n')

check_and_include_graphics(files["bacteria_count_vis",1])
```
