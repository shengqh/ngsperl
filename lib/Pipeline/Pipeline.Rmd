...
print_gsea_subfolder=function(resultDirSub, is_positive){
  pstr = ifelse(is_positive, "pos", "neg")
  title = ifelse(is_positive, "Positive-regulated", "Negative-regulated")
  pattern = paste0("gsea_report_for_na_", pstr, "_\\d+\\[.xls|.tsv]$")
  ...
  if (length(posTableFile)!=1) {
    ...
    if(!is.null(rawTableOut)){
      ename<-paste(gname, gfolders$GseaCategory[j], title)
  ...
  for (j in 1:length(comparisons)){
    ...
    for (i in 1:nrow(comp_files)){
      ...
      for (j in 1:nrow(gfolders)){
        ...
        print_gsea_subfolder(resultDIrSub, TRUE)
        print_gsea_subfolder(resultDIrSub, FALSE)
      }
    }  
  }
}
