...
print_gsea_subfolder=function(resultDirSub, gname, gseaCategory, is_positive){
  ...
  if (length(gseaTableFile)!=1) {
    ...
    rawTableOut<-processGseaTable(gseaTableFile)
    if(!is.null(rawTableOut)){
  ...
  for (j in 1:length(comparisons)){
    ...
    for (i in 1:nrow(comp_files)){
      ...
      for (k in 1:nrow(gfolders)){
        ...
        pos = print_gsea_subfolder(resultDirSub, gname, gseaCategory, TRUE)
        neg = print_gsea_subfolder(resultDirSub, gname, gseaCategory, FALSE)
        ...
        sig_file=paste0(gname, ".", gseaCategory, ".csv" )
        ...
        png(paste0(sig_file, ".png"), width=width, height=height, res=300)
        print(g)
        dev.off()
      }
