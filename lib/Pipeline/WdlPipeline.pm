...
sub addPairedFastqToUnmappedBam {
  ...
  return ($task);
} ## end sub addPairedFastqToUnmappedBam
...
sub addPairedFastqToProcessedBam {
  ...
  push @$individual, $task;
  return ($task);
} ## end sub addPairedFastqToProcessedBam
...
sub addUmiReadsToProcessedBam {
  ...
  $config->{$task} = {
    ...
    "input_list"     => { "PreProcessingForVariantDiscovery_GATK4.flowcell_unmapped_bams_list_ref" => [ $files_ref, ".fastq" ] },
    output_file_ext  => "." . $genomeForOutputExt . ".bam",
    output_other_ext => "." . $genomeForOutputExt . ".bai",
    pbs              => {
      "nodes"    => "1:ppn=8",
  ...
  push @$individual, $task;
  return ($task);
} ## end sub addUmiReadsToProcessedBam
...
sub addMutect2Wdl {
  ...
  } ## end else [ if ($mutect2_tumor_only)]
...
sub addSomaticCNV {
  ...
  if ( $def->{"perform_somaticCNV_pon"} ) {
    ...
    $pon      = { "CNVSomaticPairWorkflow.read_count_pon_ref" => [ $somaticCNV_pon, ".pon.hdf5\$" ], };
    $interval = getValue( $def, "covered_bed" );
  } ## end if ( $def->{"perform_somaticCNV_pon"...})
  else {
    $pon      = { "CNVSomaticPairWorkflow.read_count_pon" => getValue( $def, "CNVSomaticPairWorkflow.read_count_pon" ), };
...
sub addCollectAllelicCounts {
  ...
  $config->{$task} = {
    ...
    "input_single"  => { "CollectAllelicCountsWorkflow.common_sites_ref" => [ $common_sites, ".intervals\$" ], },
  ...
  push @$individual, $task;
  return ($task);
} ## end sub addCollectAllelicCounts
...
sub addEncodeATACseq {
  ...
  $config->{$task} = {
    ...
    "wdl_file"             => $pipeline->{"wdl_file"},
    "input_json_file"      => $pipeline->{"input_file"},
    "input_parameters"     => merge_hash_left_precedent(
      $encode_atac_inputs,
  ...
  } ## end else [ if ( defined $def->{replicates...})]
  ...
  $config->{$croo_task} = {
    ...
    option      => "-n __NAME__ --croo "
        . getValue( $def, "croo", "croo" )
        . " --out_def_json "
        . getValue( $def, "croo_out_def_json" )
        . " -i __FILE__ -o __OUTPUT__
...
",
  ...
  if ( $def->{perform_croo_qc} ) {
    ...
    push @$tasks, $qc_task;
  } ## end if ( $def->{perform_croo_qc...})

  return ($croo_task);
} ## end sub addEncodeATACseq
