...
sub perform {
  ...
  my $sorted           = get_option( $config, $section, "sorted",                   1 );
  my $remove_duplicate = get_option( $config, $section, "remove_duplicate",         0 );
  ...
  my $mark_duplicate   = get_option( $config, $section, "mark_duplicate",           1 );
  my $tmp_dir = "/tmp/baiy7_\\\$\{SLURM_JOBID\}"
  my $removeDupLabel;
  if ($remove_duplicate) {
    $removeDupLabel = "true";
  }
...
##start here:
    if ( !$sorted ) {
      my $qsortedFile = $sample_name . ".qsorted.bam";
      print $pbs "
if [[ -s $inputFile && ! -s $qsortedFile ]]; then
  echo sort=`date` 
  sambamba sort -u --sort-picard -t $thread -m 80G -o ${tmp_dir}/${qsortedFile} $inputFile
fi
";
      $inputFile = ${tmp_dir}/${qsortedFile};
      #$rmlist    = $rmlist . " $qsortedFile";
    }
    ...
    if ( $remove_duplicate or $mark_duplicate ) {
      ...
      my $sortedFile = $sample_name . $rmdupResultName . ".sorted.bam";
      my $recallFile = $sample_name . $rmdupResultName . ".sorted.recall.bam";
      print $pbs "
if [[ -s $inputFile && ! -s ${tmp_dir}/${rmdupFile} ]]; then
  echo MarkDuplicates=`date` 
  java $option -jar $picard_jar MarkDuplicates I=$inputFile O=${tmp_dir}/${rmdupFile} ASSUME_SORTED="queryname" COMPRESSION_LEVEL=0 M=${rmdupFile}.metrics
  echo sort=`date` 
  sambamba sort -u -t $thread -m 80G -o ${tmp_dir}/${sortedFile} ${tmp_dir}/${rmdupFile}
  echo BaseRecalibrator=`date`
  java $option -jar $gatk_jar -T BaseRecalibrator -nt 4 -R $faFile -I ${tmp_dir}/${sortedFile} -o ${tmp_dir}/${sample_name}.table $knownsitesvcf --preserve_qscores_less_than 6 -dfrac .1 -L chr1 -L chr2 -L chr3 -L chr4 -L chr5 -L chr6 -L chr7 -L chr8 -L chr9 -L chr10 -L chr11 -L chr12 -L chr13 -L chr14 -L chr15 -L chr16 -L chr17 -L chr18 -L chr19 -L chr20 -L chr21 -L chr22
  echo PrintReads=`date`
  java $option -jar $gatk_jar -T PrintReads -nt 4 -R $faFile -I ${tmp_dir}/${sortedFile} -o $recallFile -BQSR ${tmp_dir}/${sample_name}.table -preserveQ 6 -SQQ 10 -SQQ 20 -SQQ 30 --disable_indel_quals
  samtools flagstat $recallFile > ${recallFile}.stat
  fi
";
