...
figureRmd<-function(obj_map, samples_names){
  ...
  for(cur_sample in samples_names){
    ...
    decontX_png<-file.path(detail_folder, paste0(cur_sample, ".decontX.png"))
    ...
    if(has_signacx){
      ...
      signacx_png=gsub(".meta.rds", ".SignacX.dot.png", signacx_map[[cur_sample]])
      result<-paste0(result, getFigure(signacx_png, FALSE))
    }

    if(has_singler){
      result<-paste0(result, "\n### SingleR: Cell types > 1%\n\n")
      singler_png=gsub(".meta.rds", ".SingleR.dot.png", singler_map[[cur_sample]])
      result<-paste0(result, getFigure(singler_png, FALSE))
    }

    if(has_azimuth){
      result<-paste0(result, "\n### Azimuth: Cell types > 1%\n\n")
      azimuth_png=gsub(".meta.rds", ".Azimuth.dot.png", azimuth_map[[cur_sample]])
      result<-paste0(result, getFigure(azimuth_png, FALSE))
    }
