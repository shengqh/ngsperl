...
figureRmd<-function(cts, files){
  ...
  for(idx in c(1:length(celltypes))){
    ...
    for(idx in 1:nrow(ct_files)){
      fname = ct_files$type[idx]
      fpath = ct_files$file[idx]
...
d_tbl=dcast(rename_tbl, Name~Key, value.var="Value", fun.aggregate=function(x){paste0(x, collapse=",")})
d_tbl<-d_tbl[,c("from","cluster","to")]
output_table(d_tbl, "Rename/delete sub clusters")
```
