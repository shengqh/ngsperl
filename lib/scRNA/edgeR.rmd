...
file_prefix=ifelse( is_one(myoptions$DE_by_cell), 
                    paste0(outFile, ".edgeR_by_cell/", outFile, "."), 
                    paste0(outFile, ".edgeR_by_sample/", outFile, "."))
...
in_cluster_rmd<-function(edgeR_files, comparisons){
  ...
  for(comp in comparisons){
    ...
    cts = as.character(rev(comp_files$cellType))
    ct<-cts[1]
    ...
    for(ct in cts){
      ...
      mds_png<-paste0(file_prefix, ct_comp_name, ".mds.png")
      if(file.exists(mds_png)){
        result<-paste0(result,"\n### MDS\n\n")
        result<-paste0(result, getFigure(mds_png))
      }

      heatmap_png<-paste0(file_prefix, ct_comp_name, ".log_cpm.heatmap.png")
      ...
      if(nrow(sig_data) == 0){
        ...
        top_gene_png<-paste0(file_prefix, ct_comp_name, ".top_1_gene.png")
