...
if(file.exists("fileList10.txt")) {
  image_map=read_file_map("fileList10.txt", do_unlist=FALSE)
  image_slide=image_map[[sample_name]]
}else{
  image_slide=NULL
}
...
#let's get the dataType by user definition instead of guessing it.
...
if(hasAzimuth){
  ...
  stopifnot(all(rownames(azimuth_meta) %in% rownames(obj@meta.data)))

  obj@meta.data[["Azimuth"]]="Unassigned"
  obj@meta.data[rownames(azimuth_meta), "Azimuth"] = azimuth_meta$Azimuth_finest
}
```
...
if(hasRCTD){
  ...
  obj@meta.data[["RCTD1"]] = rctd_obj@meta.data[row.names(obj@meta.data),"RCTD1_spot_class"]
  obj@meta.data[["RCTD1_first_type"]] = rctd_obj@meta.data[row.names(obj@meta.data),"RCTD1_first_type"]
}
```
...
if(hasSignacX){
  ...
  obj@meta.data[["SignacX"]] = signacX_meta[row.names(obj@meta.data),"CellStates"]
}
```
