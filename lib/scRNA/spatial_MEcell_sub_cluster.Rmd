...
if (is.null(dataType) || dataType=="VisiumHD") {
  dataType=get_data_type(obj) #decide if this is a Visium, VisiumHD, or CosMx
  if (dataType!="VisiumHD") {
    warning(paste0("Data type not specified, inferred as ", dataType, " based on the object."))
  }
}
if (dataType=="CosMx") { #set fov to NULL for CosMx data
  image_slide=NULL
}
...
for(i in 1:nrow(meta_files)){
  ...
  for(cur_res in sub_resolutions){
    ...
    if(!file.exists(heatmap_png)){
      top5 = markers |>
        #dplyr::filter(p_val_adj < 0.05) |> #no need to filter marker here by p_val_adj as FindAllMarkers alreday filter marker by p value and this step may make no marker again
