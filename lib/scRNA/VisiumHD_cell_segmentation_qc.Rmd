...
```{r}
print_table(get_freq_table(rawobj@meta.data, "AssayName"), row.names=FALSE)
```
...
for(assay in assays){
  ...
  g3 = SpatialFeaturePlot(object = cur_obj, 
                          features = c(nFeatureCol), 
                          max.cutoff="q98", 
                          image.alpha = 0, 
                          pt.size.factor = pt.size.factor,
                          plot_segmentations = plot_segmentations,
  ...
  ggsave(cur_png, g, width=15, height=5, dpi=300, units="in", bg="white")
