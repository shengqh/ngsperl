...
if(isCalc){
  ...
  if(file.exists("fileList3.txt")){
    ...
    pooledObjs = lapply(poolNames, function(pn){
      ...
      cellids<-unlist(lapply(curObjs, function(x) x@project.name))
      curobj <- merge(curObjs[[1]], y = unlist(curObjs[2:length(curObjs)]), add.cell.ids = cellids, project = "integrated")
