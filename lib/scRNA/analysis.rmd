...
``` {r umap_cells, fig.height=8, fig.width=18, fig.cap=figRef("umap_cells", "UMAP of cells (left) and samples (right)")}
...
```{r sample_cells_count, fig.height=8, fig.width=18, fig.cap=figRef("sample_cells_count", "Percentage of cell from samples in each cluster")}
...
SCLC<-add_cluster(SCLC, new.cluster.ids, "cell_expression_clusters")
...
SCLC[["cellactivity_clusters"]] <- new.cluster.ids[unlist(SCLC[["seurat_clusters"]])]

clusterDf<-data.frame(seurat=unlist(SCLC[["seurat_clusters"]]), cellactivity=unlist(SCLC[["cellactivity_clusters"]]))
...
SCLC[["seurat_cellactivity_clusters"]] <- seurat_cellactivity

```

```{r gene_activity_umap, fig.width=18, fig.height=10, results="asis"}
cat("\n\n### Predicted cell types for each cluster\n\n")
p1<-DimPlot(object = SCLC, reduction = 'umap', label=T, group.by="seurat_clusters", cols=seurat_colors)
p2<-DimPlot(object = SCLC, reduction = "umap",label=T, group.by="seurat_cellactivity_clusters", cols=seurat_cellactivity_colors)
plot_grid(p1,p2)
```

```{r gene_activity_heatmap, fig.width=18, fig.height=max(10, length(top10$gene) / 10), results="asis"}
DoHeatmap(SCLC, features = top10$gene, group.by="seurat_cellactivity_clusters", group.colors=seurat_cellactivity_colors, angle=90) + NoLegend()
```
