...
if(isCalc){
  ...
  write.csv(data.norm, file=paste0(prefix, ".cluster.norm.csv"))
...
print(kable_styling(kable(cta_table, caption=tabRef("cta_table", "Cell type prediction result"))))
```
...
DimPlot(obj, reduction = "umap", label=TRUE, group.by="seurat_cellactivity_clusters", cols=seurat_cellactivity_colors, split.by = "orig.ident")
```
