...
has_html=file.exists('fileList3.txt')

bar_width = max(5, nrow(output) * 0.4 + 1)
bar_height = 6
bar_x_angle = 60
```
...
```{r eval="Reads Mapped Confidently to the Filtered Probe Set" %in% colnames(output)}
cat("\n\n## Percentage of Reads Mapped to Filtered Probe Set\n\n")
...
g=ggplot(output, aes(x=`Sample ID`, y=`Reads Mapped Confidently to the Filtered Probe Set` * 100)) +
  ...
  theme(
    ...
    axis.text.x  = element_text(size = 16, angle = bar_x_angle, hjust = 1),  # x-axis labels
...
ggsave(mapping_png, plot = g, width = bar_width, height = bar_height, units = "in", dpi = 300, bg="white")
...
ggsave(sequencing_png, plot = g, width = bar_width, height = bar_height, units = "in", dpi = 300, bg="white")
...
ggsave(binned_mean_png, plot = g, width = bar_width, height = bar_height, units = "in", dpi = 300, bg="white")
...
html_map=read_file_map("fileList3.txt", do_unlist=FALSE)
