...
cts_cluster=read.csv(parFile2, stringsAsFactors = F, header=T, row.names=1)
...
if(recluster_celltypes != ""){
  ...
  recluster_def<-read.table(parSampleFile1)
  cts<-split(recluster_def$V1, recluster_def$V2)
  cclist=list()
  ct=names(cts)[1]
  for (ct in names(cts)){
    ct_clusters = unlist(cts[ct])
    cells=rownames(cts_cluster)[cts_cluster$seurat_clusters %in% ct_clusters]
    cclist[[ct]]=cells
  }
}
...
for(ct in names(cclist)){
  ...
  if(!file.exists(rdsFile)){
    ...
    sumcounts=get_group_count(cluster_obj,"seurat_clusters")
    ...
    predict_celltype<-ORA_celltype(data.norm,celltype$cell_activity_database$cellType,celltype$cell_activity_database$weight)
  ...
  p<-DimPlot(object = cluster_obj, reduction = 'umap', label=T, group.by="seurat_cellactivity_clusters", split.by="orig.ident", combine=F)
