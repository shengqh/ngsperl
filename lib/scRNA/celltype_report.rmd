...
options_table<-read.table("fileList4.txt", sep="\t", header=F, stringsAsFactors = F)
...
finalList<-readRDS(file=filelist1[1,1])

filelist2<-read.table("fileList2.txt", header=F)
celltype<-read.csv(file=filelist2[1,1])

filelist3<-read.table("fileList3.txt", header=F)
celltype_db<-readRDS(file=filelist3[1,1])
...
obj[["cellactivity_clusters"]] <- unlist(new.cluster.ids[unlist(obj[["seurat_clusters"]])])
