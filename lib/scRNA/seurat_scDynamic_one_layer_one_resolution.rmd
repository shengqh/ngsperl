...
prefix=paste0("details/", outFile)

files<-read.csv(paste0(prefix, ".iter_png.csv"), row.names=1)
...
```{r, echo=FALSE}
check_and_include_graphics(paste0(outFile, ".layer4.umap.png"))
```
...
```{r, echo=FALSE}
check_and_include_graphics(paste0(outFile, ".layer4.dot.sub.png"))
```
    ...
    if(!by_individual_sample){
      for(group.by in c("sample", "batch")) {
        if(file.exists(paste0(outFile, ".", cell_identity, ".", group.by, "_celltype.csv"))){
          ...
          result<-paste0(result, getFigure(paste0(outFile, ".", cell_identity, ".", group.by, ".", pct_name, ".cell.png"), in_details))
        }
      }
    }
  }
