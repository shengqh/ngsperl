...
sub perform {
  ...
  for my $comparisonName ( sort keys %{$comparisons} ) {
    ...
    my $condition1treat   = $group_sample_treat{ $groupNames[0] }->[0][1];
    my $condition1control = $group_sample_control{ $groupNames[0] }->[0][1];
    my $condition2treat   = $group_sample_treat{ $groupNames[1] }->[0][1];
    my $condition2control = $group_sample_control{ $groupNames[1] }->[0][1];
