#!/usr/bin/perl
package Samtools::MappedReadNames;
...
sub perform {
  ...
  for my $sample_name ( sort keys %raw_files ) {
    ...
    my $nameFile = $sample_name . ".mnames";

    my $pbs = $self->open_pbs( $pbs_file, $pbs_desc, $log_desc, $path_file, $result_dir, $nameFile );
    print $pbs "samtools view -F 4 $bam_file | cut -f1 > $nameFile
...
sub result {
  ...
  my $result = {};
  for my $sample_name ( sort keys %raw_files ) {
    my $nameFile = $result_dir . "/" . $sample_name . ".mnames";
