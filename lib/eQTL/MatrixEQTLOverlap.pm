#!/usr/bin/perl
package eQTL::MatrixEQTLOverlap;
...
sub perform {
  ...
  my $input_files = get_raw_files( $config, $section );
  my $input_bim_files = get_raw_files( $config, $section, "source_bim_files" );
  my $input_name = get_option($config, $section, "source_name");
  
  my $compare_files = get_raw_files( $config, $section, "compare_files" );
  my $compare_bim_files = get_raw_files( $config, $section, "compare_bim_files" );
  my $compare_name = get_option($config, $section, "compare_name");
  
  my $script = dirname(__FILE__) . "/MatrixEQTLOverlap.py";
  if ( !-e $script ) {
  ...
  for my $sampleName ( keys %$input_files ) {
    ...
    my $input_file    = $input_files->{$sampleName}[0];
    my $input_bim_file= $input_bim_files->{$sampleName}[0];
...
sub result {
  ...
  for my $sampleName (keys %$snp_genotype_files) {
    my @result_files = ();
    push( @result_files, "$result_dir/${prefix}${sampleName}.comparison.tsv");
