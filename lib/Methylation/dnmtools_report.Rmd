...
png(file=paste0(project, "/", project, ".abismal.mapping.csv.png"), height=1500, width=width, res=300)
...
png(file=paste0(project, "/", project, ".coverage.stat.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "/", project, ".coverage.depth.perc.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "/", project, ".dup.rate.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "/", project, ".bs.overall.rate.csv.png"), height=1500, width=width, res=300)
...
png(file=paste0(project, "/", project, ".bs.CpG.rate.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "/", project, ".fractional_meth.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "/", project, ".mean_meth.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "/", project, ".mean_meth_weighted.csv.png"), height=1500, width=width, res=300)
...
dmcpg.all <- list.files(path = MethylKitDiff_path, pattern = dmcpg.pat, all.files = FALSE, recursive = T, ignore.case = FALSE, include.dirs = FALSE)

dmcpg.lst <- list()
for (i in 1:length(dmcpg.all)) {
	#read in *.meth files
	dmcpg.infile <- paste(MethylKitDiff_path, dmcpg.all[i], sep = "/")
	comparison <- gsub("(.*)\\/.*\\.dmcpgs$", "\\1", dmcpg.all[i])
	sample <- gsub(".*\\/(.*)\\.dmcpgs", "\\1", dmcpg.all[i])
	sample <- str_replace(sample, paste0(comparison, "_"), "")
	
	#format cpg data frame
	DT.dmcpg <- read.table(dmcpg.infile, header = T, sep = "\t", stringsAsFactors = FALSE)
	hypo_CpGs_num <- nrow(DT.dmcpg)
	link <- paste0("[",sample,"](./", project, "/", comparison, "_", sample, ".dmcpgs)")
	info <- c(comparison, sample, hypo_CpGs_num, link)
	dmcpg.lst[[i]] <- info
}

dmcpg_df <- Reduce(rbind, dmcpg.lst)
colnames(dmcpg_df) <- c("Comparison", "Hypo_in_Sample", "Hypo_meth_num", "Links")
