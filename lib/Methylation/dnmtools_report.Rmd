...
dnmtoolsdiff_path <- myoptions$dnmtoolsdiff_path
...
files <- list.files(path = abismal_path, pattern = pat, all.files = TRUE, recursive = TRUE, ignore.case = FALSE, include.dirs = FALSE)

DT <- list()

for (i in 1:length(files) ) {
  infile <- paste(abismal_path, files[i], sep = "/")
...
write.csv(file=paste0(project, "_files/", project, ".abismal.mapping.csv"), mapping_df)
...
png(file=paste0(project, "_files/", project, ".abismal.mapping.csv.png"), height=1500, width=width, res=300)
...
files <- list.files(path = dnmtools_path, pattern = pat, all.files = TRUE, recursive = TRUE, ignore.case = FALSE, include.dirs = FALSE)
samples <- gsub(".+\\/(.*).levels", "\\1", files)
DT <- list()

for (i in 1:length(files) ) {
  infile <- paste(dnmtools_path, files[i], sep = "/")
  info <- read.delim(infile, header = F, sep = " ", stringsAsFactors = FALSE, comment.char = "#", blank.lines.skip = TRUE, as.is = T)
  info <- info %>%
...
write.csv(file=paste0(project, "_files/", project, ".coverage.stat.csv"), levels_df)
...
png(file=paste0(project, "_files/", project, ".coverage.stat.csv.png"), height=1500, width=width, res=300)
...
write.csv(file=paste0(project, "_files/", project, ".HS.matrics.csv"), hs_met_df)
...
width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "_files/", project, ".coverage.depth.perc.csv.png"), height=1500, width=width, res=300)
...
write.csv(file=paste0(project, "_files/", project, ".dup.rate.csv"), dup_df)
...
width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "_files/", project, ".dup.rate.csv.png"), height=1500, width=width, res=300)
...
write.csv(file=paste0(project, "_files/", project, ".bs.overall.rate.csv"), bs_overall_df)

width=max(2000, 70 * ncol(levels_df))
png(file=paste0(project, "_files/", project, ".bs.overall.rate.csv.png"), height=1500, width=width, res=300)
...
write.csv(file=paste0(project, "_files/", project, ".bs.CpG.rate.csv"), bs_overall_df)
...
png(file=paste0(project, "_files/", project, ".bs.CpG.rate.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "_files/", project, ".fractional_meth.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "_files/", project, ".mean_meth.csv.png"), height=1500, width=width, res=300)
...
width=max(2000, 70 * ncol(rrbs_met_df))
png(file=paste0(project, "_files/", project, ".mean_meth_weighted.csv.png"), height=1500, width=width, res=300)
...
saveRDS(cpg.all.df, file = paste0(project, "_files/", project, ".all.cpg.mincov", mincov, ".rds"))
...
pdf(file = paste0(project, "_files/", project, "methyl_CpG_bvalue_cor_MDS_plot.pdf"), width = 6, height = 6, pointsize = 8, onefile = T)
dms_plot
invisible(dev.off())

png(file=paste0(project, "_files/", project, "methyl_CpG_bvalue_cor_MDS_plot.png"), height=1500, width=1500, res=300)
print(dms_plot)
invisible(dev.off())
...
```{r mds, include=TRUE, message = F, warning = F, fig.align="center", echo=FALSE}
dmcpg.pat <- ".dmcpgs$"

dmcpg.all <- list.files(path = dnmtoolsdiff_path, pattern = dmcpg.pat, all.files = FALSE, recursive = T, ignore.case = FALSE, include.dirs = FALSE)

dmcpg.lst <- list()
for (i in 1:length(dmcpg.all)) {
	#read in *.meth files
	dmcpg.infile <- paste(dnmtoolsdiff_path, dmcpg.all[i], sep = "/")
...
write.table(paste0(project, "_files/", project, "_dmcpg_sign_file.txt"), sep = "\t", quote = F)
