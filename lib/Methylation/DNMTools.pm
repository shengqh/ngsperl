...
if [ ! -s ${sample_name}.formatted.sam ]; then
   ...
   singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools format -f abismal -o ${sample_name}.formatted.sam $sampleFile
fi
...
if [ ! -s ${sample_name}.sam ]; then
   ...
   singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools uniq -D -S ${sample_name}.sam.dupstats ${sample_name}.formatted.sam ${sample_name}.sam
fi
...
if [ ! -s ${sample_name}.bsrate ]; then
  ...
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools bsrate -c $chrDir ${sample_name}.sam -o ${sample_name}.bsrate
fi


if [ ! -s ${sample_name}.all.meth ]; then
  echo dnmtools counts=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools counts -c $chrDir -o ${sample_name}.all.meth ${sample_name}.sam
fi

if [ ! -s ${sample_name}.levels ]; then
  echo dnmtools levels=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools levels -o ${sample_name}.levels ${sample_name}.all.meth
fi

if [ ! -s ${sample_name}.meth ]; then
  echo dnmtools sym=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools sym -m -o ${sample_name}.meth ${sample_name}.all.meth
fi


if [ ! -s ${sample_name}.hmr ]; then
  echo dnmtools hmr=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools hmr -o ${sample_name}.hmr -p ${sample_name}.hmrparams ${sample_name}.meth
fi

if [ ! -s ${sample_name}.pmr ]; then
  echo dnmtools pmr=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools hmr -partial -o ${sample_name}.pmr -p ${sample_name}.pmrparams ${sample_name}.meth
fi

if [ ! -s ${sample_name}.pmd ]; then
  echo dnmtools pmd=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools pmd -o ${sample_name}.pmd -p ${sample_name}.pmdparams ${sample_name}.meth
fi
...
do
  if [[ ! -s ${sample_name}.\${chr}.epiread && ! -s ${sample_name}.epiread ]]; then
    ...
    singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools states -c $chrDir -o ${sample_name}.\${chr}.epiread ${sample_name}.\${chr}.sam
...
if [ ! -s ${sample_name}.allelic ]; then
  ...
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools allelic -c $chrDir -o ${sample_name}.allelic ${sample_name}.epiread
fi
if [ ! -s ${sample_name}.amr ]; then
  echo dnmtools amrfinder=`date`
  singularity exec -B $mybinds /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools amrfinder -c $chrDir -o ${sample_name}.amr ${sample_name}.epiread
fi
