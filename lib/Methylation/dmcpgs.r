dmcpgs <- function(file,
  ...
  df <- df[,c("chr", "start", "end", "type", "beta_s1", "beta_s2", "pvalue1", "FDR1", "pvalue2", "FDR2", "M_s1", "U_s1", "M_s2", "U_s2")]
...
write.table(res_lst[[1]], file = paste0(names(res_lst)[1], ".dmcpgs"), sep = "\t", row.names = F, quote = F)
write.table(res_lst[[2]], file = paste0(names(res_lst)[2], ".dmcpgs"), sep = "\t", row.names = F, quote = F)
