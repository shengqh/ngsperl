...
if [[ ! -s $result_file_raw && ! -s $result_file_bam ]]; then
  echo abismal=`date`
  singularity exec -c -e -B /home,/gpfs23,/gpfs51,/gpfs52,/panfs,/data,/dors,/nobackup,/tmp -H `pwd` /data/cqs/softwares/singularity/dnmtools.1.0.sif dnmtools abismal -t $thread -i $abismal_index $sample_files_str -s $map_stat -o $result_file_raw
