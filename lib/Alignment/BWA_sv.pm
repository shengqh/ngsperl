...
if [ ! -s $bam_file ]; then
  if [ ! -s $unsorted_bam_file ]; then
    echo bwa_mem=`date`
    bwa mem $option -R '$rg' $bwa_index $sample_files_str | samblaster -a --addMateTags | samtools view -bS -o $bam_file
  fi
";
