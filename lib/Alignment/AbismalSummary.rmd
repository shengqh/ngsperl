...
dup_width=max(5, nrow(dup_rate) * 0.15)
dup_height=4
...
rrbs_width=max(6, nrow(rrbs_final) * 0.2 + 1)
rrbs_height=3.5
...
g<-ggplot(rrbs_final, aes(x=Sample, y=Percentage, fill = Site_type)) +
  ...
  theme(axis.text.x = element_text(angle=90, vjust=0.5, size=11, hjust=0.95, face="bold"),
        axis.text.y = element_text(size=11, face="bold"),
        legend.position = "top")
ggsave(rrbs_png, g, width=rrbs_width, height=rrbs_height, units="in", dpi=300, bg="white")
```
