...
```{r}
combined=rbind(pos_df, neg_df)
ontology<-combined[,c(1,2)]
ontology<-ontology[!duplicated(ontology$Metabolite.name), ]
no_ontology= combined %>% select(-Ontology)
f<-aggregate(. ~ Metabolite.name, no_ontology, sum)
colnames(f)<-colnames(no_ontology)
final=merge(ontology, f, by="Metabolite.name")
...
There are total `r nrow(final)` metabolites. Data was saved to `r combined_file`.
...
final_anno=dplyr::left_join(final, anno_slim, join_by(Ontology==Abbreviation)) %>%
  ...
  select(c("Metabolite.name", "Abbreviation", "Lipid_subclass","Main_class","Categories"), everything()) %>%
...
samples=colnames(final_anno)[6:ncol(final_anno)]
...
log_scale<-function(df, samples, data_prefix, box_width, box_height){
  ...
  return(list(raw_csv=raw_csv, log=ldf, log_csv=getAbsolutePath(log_csv), log_scaled=sldf, log_scaled_csv=getAbsolutePath(log_scaled_csv), log_scaled_png=log_scaled_png))
}
...
final_files=data.frame("File"=lst$log_csv, "Name"="Species")
```
...
```{r}
final_sub_class=final_anno[,c(2,6:ncol(final_anno)),drop=FALSE]
...
final_files=rbind(final_files, data.frame("File"=lst$log_csv, "Name"="Sub_class"))
```
...
final_main_class=final_anno[,c(4,6:ncol(final_anno)),drop=FALSE]
...
final_files=rbind(final_files, data.frame("File"=lst$log_csv, "Name"="Main_class"))
```
...
```{r fig.width=box_width, fig.height=box_height}
final_category=final_anno[,c(5,6:ncol(final_anno)),drop=FALSE]
...
final_files=rbind(final_files, data.frame("File"=lst$log_csv, "Name"="Category"))
```
...
```{r}
write.table(final_files, paste0(task_name, ".files.txt"), quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")
```
