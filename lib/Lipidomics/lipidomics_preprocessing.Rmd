...
cat("\n\nThere", ifelse(nrow(missed)==0, "is", "are"), nrow(missed), "molecule(s) not found in MSDial annotation file.\n\n")
```
...
log_scale<-function(df, samples, data_prefix, box_width, box_height){
  ...
  g1<-get_box(sldf, samples) + xlab("Concentration") + ggtitle("post-log-scaling")
  
  g<-g0+g1+plot_layout(ncol=2)
  ...
  return(list(raw_csv=raw_csv, log_scaled=sldf, log_scaled_csv=log_scaled_csv, log_scaled_png=log_scaled_png))
}
