...
if(file.exists("fileList4.qc.txt")){
  class_rename_map=fread("fileList4.qc.txt", header=F, data.table = F)

  meta_sel = merge(meta_sel, class_rename_map, by.x="Class", by.y="V2", all.x=TRUE)
  meta_sel$V1[is.na(meta_sel$V1)] <- meta_sel$Class[is.na(meta_sel$V1)]
  meta_sel$Class <- meta_sel$V1
  meta_sel$V1 <- NULL
