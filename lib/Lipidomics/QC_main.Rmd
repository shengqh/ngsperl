...
read_files<-function(files, file_column){
  ...
  for (i in 1:nrow(files)){
    df <- data.frame(fread(files[i,file_column]), row.names=1)
...
```{r}
remove_samples<-function(files_list, meta, pattern){
...
if(myoptions$remove_sample_pattern != ""){
  ...
  raw_files_list_sel <- remove_samples(raw_files_list)
  log_files_list_sel <- remove_samples(log_files_list)
...
```{r}
melt_samples<-function(files_list_melt){
  files_list_melt <- lapply(files_list_melt, function(x){
...
result = paste0(result, "\n# PCA\n\n")
for (i in 1:length(log_files_list)){
  category = names(log_files_list)[i]
  result = paste0(result, "\n\n## ", category, "\n")
  png_file = draw_pca(data=log_files_list[[i]], groups=meta_sel$Class, category=category, prefix=paste0(task_name, ".", category))
...
for (i in 1:length(log_files_list)){
  category = names(log_files_list)[i]
  result = paste0(result, "\n\n## ", category, "\n")
  png_file = draw_pheatmap(data = log_files_list[[i]], groups = groups, annotation_colors = annotation_colors, category=category, prefix=paste0(task_name, ".", category))
...
for (i in 1:length(raw_files_list_melt)){
  ...
  top_f_genes = select_top_n(data=raw_files_list[[i]], n=5, max_n=10)
  ...
  write.csv(cur_data, paste0(task_name, ".", category, ".csv"), row.names=FALSE)
