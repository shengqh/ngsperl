...
sub perform {
  ...
  my $mateOrientation        = $config->{$section}{mateOrientation}        or die "define ${section}::mateOrientation first"; #0 (for single ends), RF (Illumina mate-pairs), FR (Illumina paired-ends), FF (SOLiD mate-pairs)
  ...
  for my $groupName ( sort keys %{$groups} ) {
    ...
    my $bamFile =  $rawFiles->{$samples[1]}[1];
...
";
    if(scalar(@samples) > 1){
    my $bamFile2 =  $rawFiles->{$samples[2]}[1];
