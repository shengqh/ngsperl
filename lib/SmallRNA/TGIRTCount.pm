...
sub perform {
  ...
  for my $sampleName ( sort keys %trnaFiles ) {
    my $bamFile   = $trnaFiles{$sampleName}->[0];
    my $otherFile = $otherFiles{$sampleName}->[0];
...
mono $cqsFile tgirt_count $option -i $bamFile --other $otherFile -g $coordinate_file $seqcountFile $fastqFile -o $countFile
