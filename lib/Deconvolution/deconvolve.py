...
# December 2021\
# Modified by Quanhu Sheng at 20221215
...
def deconvolve(scaledMixtures, scaledSigMat, deconMethod, droppedGene, biologRepName):
    ...
    if os.path.exists(savePath + coefFile):
        ...
        os.system("echo creating coefficient file")
    ...
    if deconMethod.upper() == "NUSVR":
        ...
        svDictDF.to_csv(savePath + srrName + "_supportVectors.csv", sep = ",",
                 header = True)                                                                            
    aggCoefDF.to_csv(savePath + coefFile, sep = ",", header = True, index = True) 
    return("foo")


def main(cpmThresh, mixturePath, basisMatrixPath, techrepsrrlist, deconMethod, biologRepName):
    """
